__all__ = ['MAMonitor', 'get_monitor_files', 'load_results']
...
# My custom version of a Multi-Agent monitor
class MAMonitor(Wrapper):
    EXT = "monitor.csv"
    file_handler = None
    ...
    def __init__(self, env, filename, allow_early_resets=False, reset_keywords=(), info_keywords=()):
        ...
        if filename is None:
            ...
            if not filename.endswith(MAMonitor.EXT):
                if os.path.isdir(filename):
                    filename = os.path.join(filename, MAMonitor.EXT)
                else:
                    filename = filename + "." + MAMonitor.EXT
    ...
    def step(self, action):
        ...
        if done:
            self.needs_reset = True
            ep_agents_rew = self.rewards
            eplen = len(self.rewards[0])

            # The reward for each agent is given by rX, where X is the index of the agent
            agents_rewards_keys = ['r{}'.format(i) for i in range(self.env.env.n_agents)]
            agents_rewards_value = [round(rew, 6) for rew in ep_agents_rew]
            agents_rewards_dict = dict(zip(agents_rewards_keys, agents_rewards_value))

            ep_info = agents_rewards_dict
            ep_info.update({"l": eplen, "t": round(time.time() - self.t_start, 6)})
            for key in self.info_keywords:
                ep_info[key] = info[key]

            self.episode_rewards.append(ep_agents_rew)
...
def get_monitor_files(path):
    ...
    """
    return glob(os.path.join(path, "*" + MAMonitor.EXT))
...
def load_results(path):
    ...
    if not monitor_files:
        raise LoadMonitorResultsError("no monitor files of the form *%s found in %s" % (MAMonitor.EXT, path))
    data_frames = []
    headers = []
