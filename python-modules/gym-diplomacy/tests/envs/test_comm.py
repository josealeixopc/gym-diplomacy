...
class MyLocalThreadedTCPServerInitTestCase(unittest.TestCase):
    def test_initialize_with_port(self):
        ...
        server = comm.DiplomacyThreadedTCPServer(test_port, TestHandler)
        with server:
...
class MyLocalThreadedTCPServerTestCase(unittest.TestCase):
    server: comm.DiplomacyThreadedTCPServer

    def setUp(self) -> None:
        test_port: int = 5000
        self.server = comm.DiplomacyThreadedTCPServer(test_port, TestHandler)
    ...
    def test_handle_clients_in_separate_threads(self):
        ...
        are_all_threads_same = all(x == self.server.handler.threads[0] for x in self.server.handler.threads)

        self.assertFalse(are_all_threads_same)
        self.assertEqual(3, len(self.server.handler.threads))

    def tearDown(self) -> None:
        with self.server:
            self.server.shutdown()

    class DiplomacyTCPHandlerrTestCase(unittest.TestCase):
        ...
        def setUp(self) -> None:
            ...
            self.server = socketserver.TCPServer((self.host, self.port), comm.DiplomacyTCPHandler,
